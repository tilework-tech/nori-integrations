...
### Core Implementation
- Tests use a fake `$HOME` and stubbed binaries (`npm`, `gws`, `sprite`, `gam`) in a temp directory to avoid side effects on the real system
...
- Environment variables required by sub-package setup scripts (`GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE`, `SPRITE_TOKEN`, `GAMCFGDIR`, etc.) are set to test values in the bats `setup()` function
- Teardown cleans up both `$TEST_TMPDIR` and any artifacts `setup.sh` creates inside the repo directory (`bin/`, `nori-slack-cli/dist/`)
...
- Tests verify `bin/` directory behavior: successful builds place a symlink at `bin/nori-slack` with a relative target (`../nori-slack-cli/dist/index.js`), stale symlinks are removed when builds fail, and symlinks survive multiple idempotent runs

...
- Environment variables required by sub-package setup scripts (`GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE`, `SPRITE_TOKEN`, `GAMCFGDIR`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, etc.) are set to test values in the bats `setup()` function
...
### Things to Know
- Tests verify partial-failure tolerance: when a sub-package setup fails, `setup.sh` should exit non-zero but `~/AGENTS.md` should still be written -- it just omits the failed CLI while listing the successful ones. Each of the six integrations has a dedicated partial-failure test. Additional tests verify that remaining sub-packages continue running after an earlier failure and that the summary output includes FAIL/OK status per package

...
- Each sub-package has its own `test/` directory for package-specific tests (e.g., [@/nori-sprites/test](../nori-sprites/test/), [@/nori-aws-cli/test](../nori-aws-cli/test/))
...
### Core Implementation
- Tests use a fake `$HOME` and stubbed binaries (`sprite`, `gam`, `aws`, `nori-newsletter`) in a temp directory to avoid side effects on the real system
...
- Environment variables required by sub-package setup scripts (`SPRITE_TOKEN`, `GAMCFGDIR`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `NEWSLETTER_CONFIG_FILE`) are set to test values in the bats `setup()` function
...
### Things to Know
- Tests verify partial-failure tolerance: when a sub-package setup fails, [setup.sh](../setup.sh) should exit non-zero but `~/AGENTS.md` should still be written -- it just omits the failed CLI while listing the successful ones. Each integration ([@/nori-sprites](../nori-sprites/), [@/nori-gam](../nori-gam/), [@/nori-aws-cli](../nori-aws-cli/), [@/nori-newsletter-cli](../nori-newsletter-cli/)) has a dedicated partial-failure test. Additional tests verify that remaining sub-packages continue running after an earlier failure and that the summary output includes FAIL/OK status per package

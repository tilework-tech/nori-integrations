...
- Follows the shell-script-only pattern of [@/nori-gws](../nori-gws/) and [@/nori-sprites](../nori-sprites/) because GAM7 is already an agent-friendly CLI
- The sole executable is [setup.sh](setup.sh), a Bash script intended to be called during sprite provisioning

### How it fits into the larger codebase
- Lives in the `nori-integrations` monorepo alongside other integration packages, registered as the 4th sub-package in [@/setup.sh](../setup.sh)
- Completely separate from [@/nori-gws](../nori-gws/) -- they target different Google APIs:

| Package | Google API Surface | CLI Binary | Credential Mechanism |
|---------|-------------------|------------|---------------------|
| nori-gws | Workspace APIs (Drive, Gmail, Calendar) | `gws` | `GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE` env var |
| nori-gam | Admin SDK (users, groups, devices, domains) | `gam` | `GAMCFGDIR` env var pointing to a directory of credential files |

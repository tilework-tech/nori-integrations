...
- The validation sequence:
  ...
  2. Checks for AWS credentials using a priority cascade: environment variables (`AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`), then `~/.aws/credentials` file, then named profile (`AWS_PROFILE` + `~/.aws/config`)
...
- The credential check is existence-only -- it confirms a credential source is present but does not validate permissions. The optional `--smoke-test` flag provides deeper validation by actually calling SES
- Installation uses `npm install -g` since nori-newsletter is distributed as an npm package

...
- [@/nori-newsletter-cli](nori-newsletter-cli/) -- Setup/verification package for the `nori-newsletter` CLI for newsletter management via AWS SES (contact lists, email sending, CSV import, rate throttling); shell-script-only pattern with npm-based installation and JSON config file validation

### Things to Know
- Integrations follow two architectural patterns based on whether a suitable agent-friendly CLI already exists: nori-slack-cli and nori-broker-cli are full TypeScript CLI projects, while nori-gws, nori-sprites, nori-gam, nori-aws-cli, and nori-newsletter-cli are shell-script-only setup/verification layers around existing CLIs
